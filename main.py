...
def create_sign_tab(self):
    """Sign Images"""
    title = ttk.Label(self.sign_frame, text="Sign", font=("Arial", 16, "bold"))
    title.pack(pady=10)
    ...
    ttk.Entry(file_entry_frame, textvariable=self.file_path, state="readonly").pack(
        side=tk.LEFT, fill=tk.X, expand=True
    )
    ttk.Button(file_entry_frame, text="Browse", command=self.browse_file).pack(
    ...
    self.comment_text = scrolledtext.ScrolledText(
        comment_frame, height=4, bg="#404040", fg="#ffffff"
    )
    ...
    ttk.Button(self.sign_frame, text="Sign Image", command=self.sign_image).pack(pady=20)


def create_manage_tab(self):
    """Tab for managing signed images"""
    title = ttk.Label(self.manage_frame, text="Signed Images", font=("Arial", 16, "bold"))
    title.pack(pady=10)
    ...
    self.details_text = scrolledtext.ScrolledText(
        details_frame, height=6, bg="#404040", fg="#ffffff"
    )
    ...
    ttk.Button(button_frame, text="Show Details", command=self.show_details).pack(
        side=tk.LEFT, padx=5
    )
...
def create_search_tab(self):
    ...
    title = ttk.Label(self.search_frame, text="Search Images", font=("Arial", 16, "bold"))
    title.pack(pady=10)
    ...
    ttk.Button(id_search_frame, text="Search", command=self.search_by_id).pack(
        side=tk.RIGHT, padx=(5, 0)
    )
    ...
    ttk.Entry(hash_search_frame, textvariable=self.search_file_var, state="readonly").pack(
        side=tk.LEFT, fill=tk.X, expand=True
    )
    ttk.Button(hash_search_frame, text="Browse", command=self.browse_search_file).pack(
        side=tk.RIGHT, padx=(5, 0)
    )

    ttk.Button(search_frame, text="Search by Hash", command=self.search_by_hash).pack(pady=10)

    results_frame = ttk.Frame(self.search_frame)
    results_frame.pack(fill=tk.BOTH, expand=True, padx=20, pady=10)

    ttk.Label(results_frame, text="Search Results:").pack(anchor=tk.W)
    self.search_results = scrolledtext.ScrolledText(results_frame, bg="#404040", fg="#ffffff")
...
def create_metadata_tab(self):
    ...
    title = ttk.Label(
        self.metadata_frame, text="Search Hidden Metadata", font=("Arial", 16, "bold")
    )
    title.pack(pady=10)
...
The metadata is embedded in the pixel data and is invisible to regular metadata tools."""
    ...
    ttk.Entry(metadata_search_frame, textvariable=self.metadata_file_var, state="readonly").pack(
        side=tk.LEFT, fill=tk.X, expand=True
    )
    ttk.Button(metadata_search_frame, text="Browse", command=self.browse_metadata_file).pack(
        side=tk.RIGHT, padx=(5, 0)
    )

    ttk.Button(file_frame, text="Extract Metadata", command=self.extract_metadata).pack(pady=10)

    results_frame = ttk.Frame(self.metadata_frame)
    results_frame.pack(fill=tk.BOTH, expand=True, padx=20, pady=10)

    ttk.Label(results_frame, text="Metadata Results:").pack(anchor=tk.W)
    self.metadata_results = scrolledtext.ScrolledText(results_frame, bg="#404040", fg="#ffffff")
...
def sign_image(self):
    ...
    try:
        ...
        messagebox.showinfo(
            "Success", f"Image signed successfully!\nID: {image_id}\nHidden metadata embedded!"
        )
...
"""
            ...
            self.metadata_results.insert(
                tk.END, "No hidden metadata found.\nThe image was not signed using this system."
            )
    except Exception as e:
...
def refresh_list(self):
    ...
    cursor = self.conn.cursor()
    cursor.execute(
        "SELECT image_id, receiver, original_filename, timestamp FROM images ORDER BY timestamp DESC"
    )
